...
import { Alert, Col, Row, Table, Tabs, Tab } from "react-bootstrap";
import {
  getCoinById,
  getCryptoCoins,
  getCryptoCoinSentiment,
  getFavCoins,
  updateFavCoins,
...
import ChartPolarity from "../components/ChartPolarity";
...
const CoinScreen = ({ match }) => {
  ...
  const { crypto_coins, crypto_senti } = cryptoCoins;
  ...
  useEffect(() => {
    ...
    dispatch(getCryptoCoinSentiment(match.params.id));
  ...
  }, [match.params.id, fav_coins, crypto]);
            ...
            <Col md="8" style={{ paddingRight: "40px" }}>
              <Row>
                ...
                >
                  ...
                  <Tab eventKey="polarity" title="Polarity">
                    {crypto_senti?.Analysis ? (
                      <ChartPolarity
                        dataprog={crypto_senti?.Analysis}
                        crypto={crypto?.name}
                        legend={true}
                      />
                    ) : (
                  ...
                  <Tab eventKey="sentiment" title="Sentiment">
                    {crypto_senti?.Polarity ? (
                      <ChartSentiment
                        dataprog={crypto_senti}
                        crypto={crypto?.name}
                        color="red"
                        legend={true}
                      />
